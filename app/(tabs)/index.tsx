...
import { TefilatCounter } from '@/components/TefilatCounter';
import { DailyChallengeWidget } from '@/components/DailyChallengeWidget';
import { DailyHistoryWidget } from '@/components/DailyHistoryWidget';
...
import { useHebrewCalendar, PERIOD_THEMES } from '@/hooks/useHebrewCalendar';
...
export default function HomeScreen() {
  ...
  const { period } = useHebrewCalendar();
  ...
  const greeting = profile.firstName ? `Shalom, ${profile.firstName}` : 'Emunah · אמונה';
    ...
    <View style={[styles.root, { backgroundColor: colors.bg }]}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <TouchableOpacity
          style={[
            styles.focusBar,
            { borderColor: colors.border, backgroundColor: colors.bgSection },
          ]}
          onPress={() => {}}
          activeOpacity={0.85}
        >
          ...
          <Text style={[styles.focusGoal, { color: colors.textMuted }]}>
            {focusDays}/{FOCUS_GOAL}j
          </Text>
        </TouchableOpacity>
        ...
        <View style={styles.navRow}>
          <TouchableOpacity
            style={[
              styles.navBtn,
              activeIndex === 0 ? styles.navBtnDisabled : { borderColor: colors.borderAccent },
            ]}
          ...
          >
            <ChevronLeft
              size={16}
              color={activeIndex === 0 ? colors.textMuted + '40' : colors.textAccent}
            />
          </TouchableOpacity>
        ...
        <View style={styles.dotsRow}>
          {visibleCards.slice(0, Math.min(visibleCards.length, 12)).map((_, i) => (
            <View
              key={i}
              style={[
                styles.dot,
                {
                  backgroundColor:
                    i === activeIndex ? colors.textAccent : colors.border,
          ...
          {visibleCards.length > 12 && (
            <Text style={[styles.dotEllipsis, { color: colors.textMuted }]}>
              ···
            </Text>
          )}
        </View>
      ...
      <PremiumPaywall visible={paywallVisible} onClose={() => setPaywallVisible(false)} />
    </View>
  );
}
