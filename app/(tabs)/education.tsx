...
export default function EducationScreen() {
  ...
  const quizForModule = useMemo(
    () => activeModuleId !== null
      ? quizQuestions.filter((q: any) => q.moduleId === activeModuleId).slice(0, 10)
      : [],
    [activeModuleId]
  );
      ...
      <Modal visible={screen === 'lesson'} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={[styles.modal, { backgroundColor: colors.bg }]}>
          ...
          <ScrollView contentContainerStyle={styles.lessonContent} showsVerticalScrollIndicator={false}>
            {activeLesson?.hebrewQuote && (
              <View style={[styles.quoteBox, { backgroundColor: colors.bgSection }]}>
                <Text style={[styles.quote, { color: colors.textAccent }]}>{activeLesson.hebrewQuote}</Text>
                {activeLesson.hebrewSource && (
                  <Text style={[styles.quoteSource, { color: colors.textMuted }]}>{activeLesson.hebrewSource}</Text>
                )}
              </View>
            )}
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.titleRow}>
          <View>
            ...
            <Text style={[styles.pageSubtitle, { color: colors.textMuted }]}>Judaïsme · Torah & Sagesse</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  quote: { fontSize: 22, fontFamily: 'serif', textAlign: 'center', lineHeight: 36 },
