...
const TIPS = [
  ...
  { size: 'large' as const,  emoji: '🙏', label: 'Mois de développement', price: '25,00€', desc: 'Que Hashem vous bénisse' },
];

const MISSION_POINTS = [
  { emoji: '✨', text: 'Préserver et transmettre la sagesse millénaire de la Torah, des sages et des maîtres' },
  { emoji: '📱', text: 'Rendre accessible l\'authenticité spirituelle juive face aux distractions modernes' },
  { emoji: '🌍', text: 'Promouvoir les vraies valeurs dans un univers digital souvent superficiel' },
  { emoji: '📚', text: 'Continuer d\'enrichir le contenu avec de nouveaux événements historiques, sages et enseignements' },
];
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.verseBox, { backgroundColor: colors.bgSection, borderColor: colors.border }]}>
            <Text style={[styles.verseHebrew, { color: accent }]}>צְדָקָה תַּצִּיל מִמָּוֶת</Text>
            <Text style={[styles.verseFr, { color: colors.textMuted }]}>La charité sauve de la mort. — Mishley 10:2</Text>
          </View>
          ...
          {donated && (
            <View style={[styles.successBox, { backgroundColor: 'rgba(201,168,76,0.08)', borderColor: 'rgba(201,168,76,0.3)' }]}>
              ...
              <Text style={[styles.successText, { color: accent }]}>
                Que Hashem vous bénisse — Merci pour votre don de {donated}
              </Text>
            </View>
          )}
          ...
          <TouchableOpacity
            ...
            onPress={() => Linking.openURL('mailto:support@emunah-app.fr?subject=Emunah%20-%20Retour%20utilisateur')}
            activeOpacity={0.8}
          >
            <Mail size={18} color={accent} />
            <View style={{ flex: 1 }}>
              <Text style={[styles.contactTitle, { color: colors.textPrimary }]}>Écrire à l'équipe</Text>
              <Text style={[styles.contactSub, { color: colors.textMuted }]}>support@emunah-app.fr</Text>
            </View>
            <ChevronLeft size={16} color={colors.textMuted} style={{ transform: [{ rotate: '180deg' }] }} />
          </TouchableOpacity>

          <View style={styles.footer}>
            <Text style={[styles.footerStar, { color: 'rgba(201,168,76,0.25)' }]}>✦</Text>
            <Text style={[styles.footerText, { color: colors.textMuted }]}>
              Que ce projet soit un acte de tikun olam permanent
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  title: { fontFamily: 'FrankRuhlLibre_700Bold', fontSize: 24, marginBottom: 12, textAlign: 'center' },
  ...
  verseHebrew: { fontFamily: 'FrankRuhlLibre_700Bold', fontSize: 20, textAlign: 'center' },
  ...
  tipPrice: { fontFamily: 'FrankRuhlLibre_700Bold', fontSize: 18, marginBottom: 4 },
