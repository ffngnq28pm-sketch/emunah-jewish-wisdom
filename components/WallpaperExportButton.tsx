...
export function WallpaperExportButton({ targetRef, format = 'wallpaper', compact = false }: Props) {
  ...
  if (compact) {
    return (
      <TouchableOpacity
        style={[styles.compact, { borderColor: 'rgba(201,168,76,0.35)', backgroundColor: 'rgba(201,168,76,0.07)' }]}
        onPress={handlePress}
        disabled={loading}
        activeOpacity={0.75}
      >
  ...
  return (
    <View ref={internalRef}>
      <TouchableOpacity
        style={[styles.btn, { backgroundColor: 'rgba(201,168,76,0.1)', borderColor: 'rgba(201,168,76,0.3)' }]}
        onPress={handlePress}
        disabled={loading}
        activeOpacity={0.8}
      >
...
const styles = StyleSheet.create({
  ...
  btnTitle: { fontFamily: 'FrankRuhlLibre_700Bold', fontSize: 14, marginBottom: 2 },
