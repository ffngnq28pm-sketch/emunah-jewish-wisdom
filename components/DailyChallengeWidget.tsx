...
const CATEGORY_COLORS: Record<string, string> = {
  'tefilah':          '#4A7A8A',  // bleu azur — prière
  'gemilut-chasadim': '#4A7A5A',  // vert chesed — bonté
  'talmud-torah':     '#8B4513',  // terre de Sienne — étude
  'teshuvah':         '#6B5FA5',  // violet — repentir
  'shabbat':          '#C9A84C',  // or de Jérusalem — Shabbat
};

export function DailyChallengeWidget() {
  const { colors } = useTheme();
  const { todayChallenge: challenge, completedToday: isCompleted, streak, markComplete: markDone } =
    useDailyChallenge();
      ...
      <View style={styles.body}>
        <View style={styles.headerRow}>
          ...
          {streak > 0 && (
            ...
            >
              <Text style={[styles.countText, { color: accent }]}>{streak}🔥</Text>
            </View>
          )}
        </View>

        <Text style={[styles.challengeTitle, { color: colors.textAccent }]}>
          {challenge.title}
        </Text>
